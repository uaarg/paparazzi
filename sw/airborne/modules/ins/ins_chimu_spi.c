  ...
  while (InsLink(ChAvailable())) {
    ...
    if (CHIMU_Parse(ch, 0, &CHIMU_DATA)) {
      ...
      if(CHIMU_DATA.m_MsgID==CHIMU_Msg_3_IMU_Attitude) {
        ...
        EstimatorSetAtt(CHIMU_DATA.m_attitude.euler.phi + INS_ROLL_NEUTRAL_DEFAULT, CHIMU_DATA.m_attitude.euler.psi, CHIMU_DATA.m_attitude.euler.theta + INS_PITCH_NEUTRAL_DEFAULT);
